...
class Scannetpp(BaseRGBDDataset):
    ...
    def __init__(self, **kwargs):
        ...
        self.is_resized = True # Always resize because of depth has much lower resolution

    def get_rgb_paths(self) -> List[str]:
        path_str = str(self.base_path / "data" / self.scene / "iphone" / "rgb" / "frame*.jpg")
        rgb_paths = natsorted(glob.glob(path_str))
        return rgb_paths

    def get_depth_paths(self) -> List[str]:
        path_str = str(self.base_path / "data"  / self.scene / "iphone" / "depth" / "frame*.png")
